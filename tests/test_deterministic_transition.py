...
def test_linear_returns_standard_parameter_groups():
    transition = Linear(intercept=1.0, slope=0.5)
    ...
    assert set(result["fixed_params"]) == {"intercept", "slope"}
...
def test_linear_uses_deterministic_defaults_as_inferred_priors():
    ...
    assert set(result["hyper_params"]) == {"intercept", "slope"}
    assert result["hyper_params"]["intercept"].shape == (4,)
    assert result["hyper_params"]["slope"].shape == (4,)
...
def test_joint_prior_accepts_deterministic_transitions():
    result = JointPrior(x=Linear(intercept=0.0, slope=1.0)).sample(batch_size=3, num_steps=4)
