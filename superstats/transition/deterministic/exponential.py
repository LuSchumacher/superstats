"""Exponential deterministic transition."""
...
class Exponential(DeterministicTransition):
    """Deterministic exponential transition with an intercept and rate.
    ...
    beta            : float, Prior, or None, optional, default: None
        Exponential rate. Positive values produce growth and negative values
        produce decay. A `Prior` samples one rate per trajectory; `None` uses
        the deterministic default prior.
    ...
    Notes
    -----
    The trajectory is defined as ``intercept * exp(beta * t)``. The `sample`
    ...
    def __init__(
        ...
        self.transition_name = "exponential"

    def sample(self, batch_size: int, num_steps: int) -> Dict[str, Any]:
        """Draw `batch_size` exponential trajectories of length `num_steps`.
        ...
        trajectory = intercept[:, None] * np.exp(beta[:, None] * index[None, :])
    ...
    def sample_from_parameters(
        ...
        """Generate exponential trajectories from resolved parameters.
