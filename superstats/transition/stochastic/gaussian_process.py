...
def sample_gaussian_process(
    local_params: np.ndarray, start: np.ndarray, kernel_mat: np.ndarray, noise=1e-6
) -> np.ndarray:
    ...
    local_params : np.ndarray of shape (batch_size, steps) - the same
        array, filled with the raw Gaussian process rollout
    """
...
class GaussianProcess(StochasticTransition):
    ...
    `initial_prior`) and covariance from `kernel`, returns the
    result on the raw coefficient scale.
