...
def sample_rdm(
    ...
    correct_idx: np.ndarray = np.empty(0, dtype=np.int32),
    sigma_base: float = 1.0,
    dt: float = 0.001,
    max_steps: int = 10000,
) -> np.ndarray:
    """Sample from the Racing Diffusion Model (RDM) for multi-choice decisions.
    ...
    `v_diff`, a bias-scaled threshold, and noise fixed at `sigma_base`.
    All other accumulators on that trial share the disadvantaged drift,
    an unscaled threshold, and noise scaled by `sigma_diff`.
    ...
    sigma_diff : np.ndarray of shape (num_trials,)
        Noise scaling factor in [0, +inf) for all non-correct
        accumulators: their noise SD is `sigma_base * sigma_diff`. The
        correct accumulator always uses `sigma_base` directly.
    ...
    sigma_base : float, optional, default: 1.0
        Diffusion noise standard deviation of the correct accumulator.
        Fixed (not estimated per trial) for identifiability.
    ...
    sqrt_dt = np.sqrt(dt)
    use_default_correct = correct_idx.shape[0] == 0

    for i in prange(num_trials):
        t = tau[i]
        correct = 0 if use_default_correct else correct_idx[i]
